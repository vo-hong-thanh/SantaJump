 ...
 * @author vot2
 */
public class RunHeuristicSanta {
...
//                System.out.println(" location " + locations[i] +" = " + distances[i]);                    
            ...
            while (count < V) {
                ...
                if(parent[u] == 0 && count > 1)
                {
                    ...
                    int candidateU = minDistanceIndextoLastVisited(V, lastVisited, parent, distances, mstSet);
...
//                    }
                    ...
                    double utolastVisited = Location.distanceBetween(locations[u], locations[lastVisited]);
                    double lastVisitedtoCandidateU = distances[candidateU];
                    
                    System.out.println("distance between " + locations[lastVisited] + " to " + locations[u] + " = " + utolastVisited);
                    System.out.println("distance between " + locations[lastVisited] + " to " + locations[candidateU] + " = " + lastVisitedtoCandidateU);
                    
                    if(lastVisitedtoCandidateU < utolastVisited)
                    {
                        System.out.println("update " + u + " to " + candidateU); 
                        u = candidateU;
                    }
...
//        }
    ...
    private static int minDistanceIndextoLastVisited(int Vertices, int lastVisitedVertex, int[] parent, double distances[], boolean mstSet[]) {
        // Initialize min value 
        double min = Double.MAX_VALUE;
        int min_index = -1;
